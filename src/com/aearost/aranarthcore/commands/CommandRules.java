	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length > 0) {
			if (args[0].toLowerCase().equals("arena")) {
				sender.sendMessage(
						ChatUtils.translateToColor("&8                        - - - &c&lArena Rules &8- - -"));
				...
				sender.sendMessage(ChatUtils.translateToColor(
						"&4&l3. &e&oTeaming is allowed, however troll-like tactics are not permitted."));
				return true;
			} else if (args[0].toLowerCase().equals("general")) {
				sender.sendMessage(
						ChatUtils.translateToColor("&8                        - - - &6&lGeneral Rules &8- - -"));
				sender.sendMessage(
						ChatUtils.translateToColor("&e&l1. &7&oBe kind to each other, and use common sense."));
				sender.sendMessage(ChatUtils.translateToColor(
				...
				sender.sendMessage(ChatUtils.translateToColor("&e&l3. &7&oNo advertising other servers."));
				sender.sendMessage(ChatUtils.translateToColor(
						"&e&l4. &7&oNo advantage mods, or hacked clients. If you are unsure if you can use a mod, please contact a staff member."));
				sender.sendMessage(ChatUtils.translateToColor("&e&l5. &7&oSpamming is not permitted"));
				...
				sender.sendMessage(ChatUtils.translateToColor(
						"&e&l8. &7&oGold and iron farms are not permitted; neither are lag-inducing farms. Spawner farms are allowed."));
				sender.sendMessage(
